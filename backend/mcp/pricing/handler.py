"""AWS Lambda handler exposing the awslabs AWS Pricing MCP server as an
...
Only the Step-3 curated tools in ``ALLOWED_TOOLS`` are dispatchable here; the
rest of the awslabs server's surface is intentionally not exposed (the
gateway target's tools.json should match this set).
"""
...
def _extract_tool_name(context):
    ...
    if not raw:
        return None
    # Target prefix is joined with a triple underscore; the tool name itself
    # never contains '___', so split on the first occurrence.
...
def _to_jsonable(result):
    ...
    text when possible) so the model gets clean structured output.
    """
