...
The whole `CryloAgents/` scaffold + agent code is **committed**, so a teammate
...
(cd CryloAgents/app/Reasoning && uv sync)
```

`agentcore deploy` installs the CDK `node_modules/` itself on first run; if it
doesn't, `cd CryloAgents/agentcore/cdk && npm install`.
...
```bash
cd backend/agents
agentcore create --name CryloAgents --framework Strands --model-provider Bedrock --memory none
agentcore add agent --name Reasoning --framework Strands --model-provider Bedrock --memory none --language Python
```

This produces `backend/agents/CryloAgents/` with `agentcore/agentcore.json`,
...
Copies `backend/canvas_core/` into `CryloAgents/app/Reasoning/canvas_core/`.

### 4. Local dev + deploy (you run)

```bash
cd backend/agents/CryloAgents
...
├── README.md
├── vendor_canvas_core.sh
└── CryloAgents/            # generated by `agentcore create` (you run)
