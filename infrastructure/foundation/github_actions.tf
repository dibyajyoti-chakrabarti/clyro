...
data "aws_iam_policy_document" "deploy" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:cloudfront::${var.account_id}:distribution/${module.frontend.cloudfront_distribution_id}",
      "arn:aws:cloudfront::${var.account_id}:distribution/${module.frontend_admin.cloudfront_distribution_id}",
    ]
  }

  # The build reads its Vite configuration from here instead of carrying
  # hardcoded pool ids in the workflow file, which is how the old workflows
  # ended up pointing at a Cognito pool in an account that no longer exists.
  statement {
    sid       = "ReadBuildConfiguration"
    effect    = "Allow"
