  ...
  return (
    <header className='border-b border-white/[0.08] bg-background/95 text-text-primary backdrop-blur-md'>
      <div className='mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-4 sm:px-6 lg:px-8'>
        ...
        >
          <span className='flex size-9 items-center justify-center rounded-md border border-amber-300/35 bg-amber-400/10 text-amber-300 shadow-[0_0_24px_rgba(251,191,36,0.12)]'>
            <span className='size-3 rounded-[3px] border-2 border-current' aria-hidden='true' />
          </span>
          <span>Crylo</span>
        </a>

        <nav className='hidden items-center gap-1 rounded-full border border-white/[0.08] bg-white/[0.02] p-1 md:flex' aria-label='Primary navigation'>
          {navItems.map((item) => (
            <a
              key={item}
        ...
        >
          <span className='relative h-4 w-5' aria-hidden='true'>
            ...
            <span className={`absolute left-0 top-[7px] h-0.5 w-5 rounded-full bg-current transition-opacity ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`} />
            <span className={`absolute bottom-0 left-0 h-0.5 w-5 rounded-full bg-current transition-transform ${isMenuOpen ? '-translate-y-[7px] -rotate-45' : ''}`} />
          </span>
        </button>
      </div>

      <div id='mobile-navigation' className={`${isMenuOpen ? 'block' : 'hidden'} border-t border-white/[0.08] bg-background/98 md:hidden`}>
        <nav className='mx-auto flex w-full max-w-7xl flex-col gap-1 px-5 py-4 sm:px-6' aria-label='Mobile navigation'>
          {navItems.map((item) => (
            <a
              key={item}
              href={`/${item.toLowerCase() === 'products' ? '' : item.toLowerCase()}`}
